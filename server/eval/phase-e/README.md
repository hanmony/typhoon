...
> 冻结记录：v1 2026-08-20（PDF 版）｜v2 2026-08-20（数据库重拟版，本版为评估用冻结版）
...
- **v2（本版）**：按用户指令（2026-08-20）**基于本地数据库已有内容重拟 210 题**：
  - 工具路由 80：平台 agent 8 个真实工具 × 10 场景（依据 `agent.prompt.ts` 工具清单）；
...
| 项 | 值 |
|---|---|
| gold-set.v2.jsonl SHA-256 | `B5EDA9C459C54AC047FD549D47EA3A4A26A8BE397E7793EEE949ADDF22F88058` |
...
| 字段 | 口径 |
|---|---|
| `expectedTool` | 工具路由题：应路由的平台 agent 工具（8 工具 × 10 场景），数据源见工具说明 |
...
- 金标准冻结后不得静默修改；确需修改时升为 v3 并记录变更原因（v1→v2 原因见「版本说明」）。
- PDF 原文不提交（未获用户明确允许）。
