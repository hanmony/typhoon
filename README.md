...
- **执行结果（2026-08-18，全量重扫 + 同日用户决策修订）**：
  ...
  - ✅ **保留 B——官方文档 49 份**：官方 PDF 26 份（防汛汇编：预案/处置方案 8、管理规定 2、通知 7、工作指令 7；台风案例基础数据 1；轩岚诺防御通知 1、梅花防御通知 1——原预估 27 份中有 2 份与汇编内文件哈希相同已去重）+ 官方 docx 8 份（防御工作情况报告、运营保障要求、事件汇总、限速区段、正线存车方案、指标统计通知、台风汇总表等）+ 原待定 17 份（用户决策全部纳入，见下）→ 平台分类 `emergency_plan`（预案/处置方案）或 `regulation`（规定/通知/指令/报告）；
  - ⛔ **排除——敏感 18 份**（只记录路径/大小/理由，绝不读内容）：`领导身份证.pdf`、梅花值班表 10 份、轨交支援人员联系方式、汇编附件 1–6 负责人/联络员名单与应急联络表；
  - ⛔ **排除——疑似扫描件 3 份**（D3 实测无文字层，用户确认不保留）：沪汛办〔2022〕40 号、沪汛办（2023）30 号两份红头通知（扫描图片版，全文仅 2–3 字）+ 一篇 14 页论文（水印掩正文）；如需 OCR 后纳入，删除 `scan_docs.py` 对应规则重跑即可；
...
- **验收**：保留清单 = 25 学术 PDF + 49 官方文档（26 PDF + 8 docx + 17 原待定 − 2 扫描件），排除清单逐条有理由（含 3 份疑似扫描件）✅；去重按 SHA-256（比「文件名+大小」更可靠）✅；全部文件归类、零未归类 ✅。
- **预估工作量**：0.5 天。
...
- **做法**：新建 `docs_import/extract_docs.py`——只读 D2 过滤清单，逐份提取正文，输出 `docs_import/text/` 下每份文档一个 `.txt`（UTF-8，目录镜像源目录）+ `docs_import/extract_metadata.json`（文件名/路径/大小/平台分类/提取方式/字数/页数/状态）。
  ...
  - `.xls` 老格式：pandas + xlrd。
- **执行结果（2026-08-18，同日用户决策修订后重跑）**：
  - keep 批次 74 份：✅ **全部提取成功（ok=74）**——26 篇学术论文字数/页数全部合理（如 37 页论文 9.7 万字）+ 官方文档 48 份（含原待定 17 份：Word COM 提取 12 份老 `.doc` 中文保真度完美、pandas 提取 `.xls` 规章、`梅花.docx` 确认为「上海轨道交通防汛防台信息快报」22129 字）；
  - 疑似扫描件 3 份：⚠️ 判定后**按用户决策剔除**（两份沪汛办红头通知为扫描图片版仅 2–3 字、一篇 14 页论文剔除重复水印后正文为空）——不再保留 txt，D2 清单已加 `exclude_scan` 规则；
...
- **验收**：74 份全部有提取结果（74 ok）✅；字数与页数粗略匹配（学术论文全部达标）✅；3 份疑似扫描件按用户决策剔除、残留 txt 已删除 ✅。
- **预估工作量**：1 天。
