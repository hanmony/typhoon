...
#### 步骤 D2：文献与文档盘点：过滤清单
- **做法**：新建 `docs_import/scan_docs.py`——遍历数据根目录（自动下沉到最内层），按规则表把每个文件归入 7 类；保留清单按 SHA-256 内容去重；未命中任何规则的文件进 `unclassified` 并以退出码 1 结束（宁可失败也不悄悄漏文件）。输出 `docs_import/filter_manifest.json`（结构化清单，D3 直接读取）+ `docs_import/盘点清单.md`（人类可读）。
- **执行结果（2026-08-18，全量重扫）**：
  - ✅ **保留 A——学术文献 26 篇**（`文献/`，平台分类 `other`）；
  - ✅ **保留 B——官方文档 34 份**：官方 PDF 26 份（防汛汇编：预案/处置方案 8、管理规定 2、通知 7、工作指令 7；台风案例基础数据 1；轩岚诺防御通知 1、梅花防御通知 1——原预估 27 份中有 2 份与汇编内文件哈希相同已去重）+ 官方 docx 8 份（防御工作情况报告、运营保障要求、事件汇总、限速区段、正线存车方案、指标统计通知、台风汇总表等）→ 平台分类 `emergency_plan`（预案/处置方案）或 `regulation`（规定/通知/指令/报告）；
  ...
  - ⚠️ **待定 17 份**：`.doc` 老格式 12 份（梅花速报 9/停运预报 2/轩岚诺防御通知 1）、保障工作总结 docx 3 份、`.xls` 规章 1 份、文件名不明的 `梅花.docx` 1 份——第 D3 步看提取工具可用性再决定；
...
- **验收**：保留清单 = 26 学术 PDF + 34 官方文档（26 PDF + 8 docx），排除清单逐条有理由 ✅；去重按 SHA-256（比「文件名+大小」更可靠）✅；全部文件归类、零未归类 ✅。
- **预估工作量**：0.5 天。
...
- **做法**：新建 `docs_import/extract_docs.py`——只读 D2 过滤清单，逐份提取正文，输出 `docs_import/text/` 下每份文档一个 `.txt`（UTF-8，目录镜像源目录）+ `docs_import/extract_metadata.json`（文件名/路径/大小/平台分类/提取方式/字数/页数/状态）。
  ...
  - `.xls` 老格式：pandas + xlrd。
- **执行结果（2026-08-18）**：
  - keep 批次 60 份：✅ 提取成功 57 份（26 学术论文字数/页数全部合理，如 37 页论文 9.7 万字）；⚠️ 疑似扫描件 3 份（两份沪汛办红头通知是扫描图片版仅 2–3 字、一篇 14 页论文剔除重复水印后正文为空）——txt 已保留并标记 `suspect_scan`，D4 不硬啃；
  - 待定批次 17 份：✅ **全部提取成功**（`antiword` 可用；本机装有 MS Word 16，COM 提取 12 份老 `.doc` 中文保真度完美；`.xls` 经 pandas 提取）——其中 `梅花.docx` 经提取确认为「上海轨道交通防汛防台信息快报」（22129 字，台风实况/预警/响应措施，建议归 `regulation` 入库，待用户确认）；
...
- **验收**：60 份全部有提取结果或明确标记（57 ok + 3 疑似扫描件）✅；字数与页数粗略匹配（学术论文全部达标）✅；待定 17 份全部提取成功供决策 ✅。
- **预估工作量**：1 天。
