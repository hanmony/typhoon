...
- **执行结果（2026-08-18，codex 审查 D2/D3 后继续）**：
  - **产出**：`docs_import/clean_docs_text.py`（六条规则 R1–R6，参数全部来自 72 份 txt 实测普查）；`docs_import/text_clean/`（已 gitignore）与 `clean_report.json` / `clean_report.md`（每份文件逐条清洗明细，可核对）。
  - **数字**：72 份全部清洗成功——30628 行 → 11763 行、1546223 字 → 1459924 字；R1 乱码剔 `(cid)` 2151 处 + 整行删 569；R2 页码 3059 + 邻页眉 114；R3 近重复短行 1634 + 长行 293；R4 断行合并 13081 处。
