...
### M6 阶段 G-3：一键研判时间基准与交互人工验收（2026-08-21）
- **首次发现的问题**：模拟指挥场景中，线路风圈相交按 `simulateStartTime + 已运行时长` 计算，但提示词使用历史轨迹最后一点。现场出现卡片按上海附近模拟时点计算、文字却称台风已到历史末点并远离上海的矛盾，故首次验收不予通过。
- **后端修复**：新增 `AnalysisBasis`（`simulated/realtime`、查询时间、状态时间、时效标记）和统一的 `situation`；`computeLineImpact` 同时产出线路影响、时间基准及供提示词使用的状态。模拟模式禁止把历史末点表述为当前实时状态；实时模式过滤早于最新观测点的过期预报，并在数据过期时要求明确提示时效不足。
- **前端修复**：研判卡片显示“模拟演练/实时研判”和研判时点，过期数据展示时效提示。人工取消测试又发现立即中断会留下空白助手气泡；`onStop` 现删除完全空白的占位消息，已有卡片、思考或文字时则保留部分结果并结束流式状态，旧流回调继续由 `streamSeq` 隔离。
- **自动验证**：`AnalyzerService` 定向单测 14/14；后端与前端生产构建通过。前端定向 Karma 仍被仓库既有历史 spec 编译错误阻断（与本次改动无关），未扩大修改范围；新增取消场景 spec 覆盖“空白占位删除”和“部分结果保留”。
- **Edge 人工验证**：卡片显示“模拟演练”及北京时间研判时点，文字使用对应 UTC 轨迹状态，未再使用历史末点；空线路结果保留“不代表不受影响”免责声明，相似案例和等级建议正常。立即取消无空白气泡，部分输出后取消保留已有卡片且 10 秒内无迟到文本；运行按钮切换为“中断”，半屏窄窗口无横向溢出。
- **结论**：阶段 G 第 3 项通过；时间基准、卡片字段、保守措辞、取消/竞态和窄窗口显示满足本机人工验收口径。

### M6 阶段 H-1：本机服务受控重启与持久化验证（2026-08-21）
- **重启前基线**：MongoDB `kbdocuments=72`、`kbchunks=3002`、`chatsessions=4`；Qdrant `knowledge_base=3002`。选取一个已有会话作为哨兵，只记录标识与消息数 14，不读取正文。MongoDB 和 Qdrant 均确认挂载 Docker volume。
- **受控重启**：依次重启 `mongo-typhoon-test`、`qdrant-typhoon`、监听 3000 的发布后端及监听 12080 的 nginx。后端使用现有精确端口启停脚本并在约 16 秒后就绪。nginx 原进程属于另一管理员权限上下文，优雅控制与普通精确停止均被 Windows 拒绝；核对 PID 文件和配置语法后，经 UAC 授权只结束 PID 15204 进程树，再以同一程序和配置隐藏启动，新主 PID 23644，HTTP 200。
- **持久化与链路验证**：最终 MongoDB 仍为 72/3002、Qdrant 仍为 3002、会话总数仍为 4；哨兵会话仍存在且仍为 14 条消息。后端 `/doc`、nginx 页面、nginx `/api/doc` 均为 200。会话 E2E 直连后端 17/17、经 nginx 17/17，覆盖创建、列表、详情、两轮记忆、agent、落库、错误路径和删除。
- **发布验证器修复**：重启后首次 `deploy/release-verify.js` 为 6/7，研判流本身通过，失败项显示“0 条全部命中”。根因是验证器仍用整条历史轨迹直接计算，且硬性要求卡片至少一条线路，与 G-3 的统一研判时点契约不一致。现解析 analysis 事件，按 `basis.queryTime` 和 `basis.mode` 复现当前状态/预报窗口，并比较卡片与直接计算的完整线路集合及最高风圈等级；双方均为 0 时按一致通过。修复后完整发布验证 7/7。
- **已知部署项**：两个数据容器的 restart policy 当前均为 `no`。本次验证证明人工受控恢复成功，但不能据此宣称 Windows/Docker 主机重启后会自动拉起；自动启动编排在阶段 H 后续确认。
- **结论**：阶段 H-1 通过；四项服务恢复、会话持久化及 72/3002/3002 数据完整性均得到重启前后闭环证据。
