...
import { LlmModelApi } from '../../../services/apis/llm-model';
...
const QUICK_CARDS = [
  { title: '案例总结', desc: '总结当前案例关键信息' },
  { title: '工作总结', desc: '生成工作总结报告' },
  { title: '智能分类', desc: '对案例进行智能分类' },
  { title: '随便总结', desc: '随机生成分析总结' },
];

const AGENT_OPTIONS = [{ label: '防台防汛助手', value: 'flood-assistant' }];

const AUTO_OPTION = { label: 'Auto', value: 'auto' };

@Component({
  selector: 'library-ai-panel',
  standalone: true,
  imports: [LibraryNzModule, FormsModule, MarkdownComponent],
  providers: [LibraryChatService],
  templateUrl: './library-ai-panel.component.html',
  styleUrl: './library-ai-panel.component.less',
})
export class LibraryAiPanelComponent implements AfterViewChecked, OnInit, OnDestroy {
  @Input() isOpen = false;
  ...
  modelOptions = signal<{ label: string; value: string }[]>([AUTO_OPTION]);
  selectedAgent = '';
  selectedModel = 'auto';
  ...
  constructor(
    readonly chat: LibraryChatService,
    private readonly llmModelApi: LlmModelApi,
  ) {
  ...
  private async loadModelOptions() {
    try {
      const models = await this.llmModelApi.list();
      if (models.length > 0) {
        const options = models.map(m => ({ label: m.name, value: m._id }));
        this.modelOptions.set([AUTO_OPTION, ...options]);
      }
    } catch {
      // 加载失败不阻塞对话，保持只有 Auto
    }
  ...
  onSend() {
    ...
    const modelId = this.selectedModel === 'auto' ? undefined : this.selectedModel;
    const useAgent = !!this.selectedAgent;
    if (useAgent) {
      this.chat.sendAgentMessage(this.inputText, modelId);
    } else {
      this.chat.sendMessage(this.inputText, modelId);
    }
    this.inputText = '';
    this.pendingScroll = 'jump';
  }
  ...
  onQuickCard(card: { title: string }) {
    this.inputText = card.title;
  }
