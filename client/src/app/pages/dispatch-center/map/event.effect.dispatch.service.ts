...
export class DispatchEventEffectService extends SharedEventEffectService {
  initiated = false;
  map!: LeafletMap;
  ...
  // constructor(private override occEventBusService: OccEventBusService) {
  ...
  initializeRemoteDataWithMarker(evs: ExtremeOcc.Event[]) {
    ...
    const pure = evs
      .filter((ev) => ev.effectDuration === 2 || ev.effectDuration === 3)
      .map((ev) => {
  ...
  diffEventsAndEffectRemote(evs: ExtremeOcc.Event[]) {
    const effect15and30 = (ev: ExtremeOcc.Event) => {
      return ev.effect && (ev.effectDuration === 2 || ev.effectDuration === 3);
    };
    const diffResult = diffEvents(
      evs.filter(effect15and30),
      this.events.filter(effect15and30),
    );
    const { added, removed, changed } = diffResult;
    this.markRemoteRemoved(removed);
    this.markRemoteAdded(added);
    this.markRemoteChanged(changed);

    this.updateRemoteMarkerAccordingToDiff();
  }

  markRemoteAdded(evs: ExtremeOcc.Event[]) {
    evs.forEach((ev) => {
      const targetRemoteItem = this.remoteDataWithMarker.find(
        (item) => item.line === ev.line,
      );

      if (targetRemoteItem) {
        const locationDto = {
          position: targetRemoteItem.position,
          positionString: targetRemoteItem.positionString,
          meta: ev,
        };
        targetRemoteItem.data.push(locationDto);
        targetRemoteItem.added = [
          ...(targetRemoteItem.added || []),
          locationDto,
        ];
        targetRemoteItem.removeOrAdd = (targetRemoteItem.removeOrAdd || 0) + 1;
      }
    });
  }

  markRemoteRemoved(evs: ExtremeOcc.Event[]) {
    evs.forEach((ev) => {
      const targetRemoteItem = this.remoteDataWithMarker.find(
        (item) => item.line === ev.line,
      );
      if (targetRemoteItem) {
        const removed = targetRemoteItem.data.splice(
          targetRemoteItem.data.findIndex((d) => d.meta.id === ev.id),
          1,
        );
        if (removed[0]) {
          targetRemoteItem.removeOrAdd =
            (targetRemoteItem.removeOrAdd || 0) - 1;
        }
      }
    });
  }
  markRemoteChanged(changed: ExtremeOcc.EventChange[]) {
    changed.forEach((c) => {
      if (c.changes['severity']) {
        const target = this.findTargetRemoteDtaWithMarkerWithId(c.id);
        if (target) {
          target.changed = true;
        }
      }
          ...
          getDispatchCenterLineIcon(
            {
              line: item.line,
              15: item.data.filter(
                (d) => d.meta.effect && d.meta.effectDuration === 2,
              ).length,
              30: item.data.filter(
                (d) => d.meta.effect && d.meta.effectDuration === 3,
              ).length,
            },
            scale,
          ),
  ...
  renderRemoteMarkers() {
    this.remoteDataWithMarker.forEach((item) => {
      ...
      this.featureGroup.addLayer(marker);
      item.marker = marker;
      // this.dataWithMarker.push({ data: event, marker });
    });
  }
  ...
  getRemoteMarker(data: RemoteMarkerMapArrayItem) {
    ...
    const m = marker(position as LatLngExpression, {
      icon: getDispatchCenterLineIcon(
        {
          line: data.line,
          15: data.data.filter(
            (d) => d.meta.effect && d.meta.effectDuration === 2,
...
export class DashboardEventEffectService extends DispatchEventEffectService {}
