...
@Component({
  selector: 'ds-real-time-close',
  imports: [CommonModule, ModuleHeaderComponent, ScaleLineComponent],
  templateUrl: './real-time-close.component.html',
  styleUrl: './real-time-close.component.less',
})
export class RealTimeCloseComponent {
  ...
  autoScroll() {
    this.animationFrameTimer = this.animationFrameFunc(() => {
      if (this.scrollPosition < this.scrollHeight - 300 + 100) {
  ...
  updateLineCloseData() {
    ...
    this.separatedLineCloseData.forEach((line) => {
      ...
      line.closePercent = Math.floor((line.closed / line.total) * 100);
    });
